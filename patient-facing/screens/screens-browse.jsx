// Screens: Browse physicians + Physician detail (with date/time picker)
...
function BrowseScreen({ filters, setFilters, onSelect }) {
  ...
  const filtered = React.useMemo(() => PHYSICIANS.filter((p) => {
    const q = filters.q.toLowerCase();
    if (q && ![p.name, p.specialty, p.subspecialty ?? ""]
      .some((s) => s.toLowerCase().includes(q))) return false;
    if (filters.specialty !== "All specialties" && p.specialty !== filters.specialty) return false;
    if (filters.language !== "Any language" && !(p.languages ?? []).includes(filters.language)) return false;
    return true;
  }), [filters]);
  ...
  return (
    <div className="page">
      ...
      <div className="filterchips">
        ...
        <label className={"chip " + (filters.language !== "Any language" ? "is-active" : "")}
...
const shortLocation = (loc) => loc.split(" — ")[0];

const nextAvailText = (days) =>
  days === 0 ? "Today" : days === 1 ? "Tomorrow" : `In ${days} days`;

const totalSlotCount = (s) => s.morning.length + s.afternoon.length + s.evening.length;
        ...
        <div key={p.id} className="doc-card" onClick={() => onSelect(p.id)}>
          ...
          <div className="doc-card-body">
            ...
            <div className="doc-meta-row">
              <span className="it"><Icon name="pin" size={13} /> {shortLocation(p.location)}{p.distanceMi ? ` · ${p.distanceMi} km` : ""}</span>
...
function DetailScreen({ physician, draft, setDraft, onBack, onContinue, rescheduleMode }) {
  ...
  const slots = React.useMemo(
    () => slotsFor(physician.id, selectedDate),
    [physician.id, selectedDate]
  );
  const totalSlots = totalSlotCount(slots);
      ...
      <div className="detail-grid">
        ...
        <div className="card booking">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: "var(--surface-2)", borderRadius: 10, marginBottom: 16, fontSize: 13, color: "var(--ink-1)" }}>
            ...
            <span>In-person at <b style={{ color: "var(--ink-0)", fontWeight: 600 }}>{shortLocation(physician.location)}</b></span>
          </div>
          ...
          <div className="datestrip">
            {days.map((d) => {
              const iso = toISO(d);
              const has = totalSlotCount(slotsFor(physician.id, iso)) > 0;
              return (
                <button key={iso}
          ...
          {draft.time && (
            <div className="booking-selected" style={{ marginTop: 12 }}>
              ...
              <div style={{ color: "var(--ink-2)", marginTop: 2 }}>
                {shortLocation(physician.location)}
              </div>
            </div>
          )}
          ...
          <button
            ...
            onClick={onContinue}>
            {rescheduleMode
              ? <>Confirm reschedule <Icon name="check" size={16} /></>
